...
def extract_file_info(filename):
    # Regex to match Translate followed by language type, Rank, dataset size, and index
    match = re.search(r'Translate\d+-(\w+)\.Rank8\.(\d+)_(\d+)\.out$', filename)
    
    if match:
        trans_type = match.group(1)  # Translation type (e.g., koen, cnko)
        dataset_size = match.group(2)  # Dataset size (e.g., 200, 800)
        index = match.group(3)  # File index (e.g., 226, 330)
        rank = '8'  # Rank is fixed as 8
...
def process_file(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        for i, line in enumerate(lines):
            if line.startswith("다음 글을 한국어로 번역해주세요") or \
               line.startswith("다음 글을 중국어로 번역해주세요") or \
               line.startswith("다음 글을 영어로 번역해주세요") or \
               line.startswith("다음 글을 일본어로 번역해주세요") or \
               line.startswith("请将以下文章翻译成中文"):
                # Extract source (src) text, which is the line after the translation prompt
                src = lines[i+2].strip().replace("<end_of_turn>", "").strip()
            elif line.startswith("Adapter Fine-tuned Output"):
                # Extract Adapter Fine-tuned Output (mt_adapter), next line contains the output
                mt_adapter = lines[i+1].strip()
            elif line.startswith("Gemma Output"):
                # Extract Gemma Output (mt_gemma), next line contains the output
                mt_gemma = lines[i+1].strip()
            elif line.startswith("Label trans result"):
                # Extract Label Translation Result (ref), next line contains the reference
                ref = lines[i+1].strip()
