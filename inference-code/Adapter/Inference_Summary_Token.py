...
FINETUNE_MODEL = "./exper/MergeModel/summary-gemma-2b-it-sum-ko.Rank" + str(rank) + "." + dataset_size
...
pipe_finetuned = pipeline("text-generation", model=finetune_model, tokenizer=tokenizer, min_new_tokens=100, max_new_tokens=100)
...
pipe = pipeline("text-generation", model=model, tokenizer=tokenizer, min_new_tokens=100, max_new_tokens=100)
...
for i in range(test_num):
    ...
    reference = doc['summary2'][i]
    
    output_file = "./exper/InferenceResult/Summary_Token100.Rank" + str(rank) + "." + dataset_size + "_" + str(i) + ".out"
...
output_report = "./exper/InferenceReport/Summary_Token100_Inference.Rank" + str(rank) + "." + dataset_size + ".out"
f2 = open(output_report, "w")
f2.write("Dataset: Summary News\n")
