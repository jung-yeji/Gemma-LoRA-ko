...
FINETUNE_MODEL = "./exper/MergeModel/summarySEMINAR-gemma-2b-it-sum-ko.Rank" + str(rank) + "." + dataset_size
...
test_dataset = "datasetSEMINAR-400"
...
for i in range(test_num):
    messages = [
        {
            "role": "user",
            "content": "다음 회의의 대화를 한문장으로 요약해주세요:\n\n{}".format(doc['passage'][i])
        }
    ]
    ...
    if int(is_gemma_train) == 1:
        ...
        Gemma_file_name = "./exper/gemma_out/Gemma_SummarySEMINAR_output_" + str(i) + ".txt"
        Gemma_file = open(Gemma_file_name, "w")
        Gemma_file.write(Gemma_out)
        Gemma_file.close()
        Gemma_time_name = "./exper/gemma_out/Gemma_SummarySEMINAR_time_" + str(i) + ".txt"
    ...
    reference = doc['summary1'][i]
    
    output_file = "./exper/InferenceResult/SummarySEMINAR.Rank" + str(rank) + "." + dataset_size + "_" + str(i) + ".out"
...
output_report = "./exper/InferenceReport/SummarySEMINAR_Inference.Rank" + str(rank) + "." + dataset_size + ".out"
f2 = open(output_report, "w")
f2.write("Dataset: Summary SEMINAR\n")
