...
FINETUNE_MODEL = "./exper/FullFTModel/FFTModel-QA." + dataset_size
...
test_dataset = "dataset-QAequal-400"
...
for i in range(test_num):
    messages = [
        {
            "role": "user",
            "content": "다음 질문에 대답해주세요:\n\n{}".format(doc['question'][i])
        }
    ]
    ...
    if int(is_gemma_train) == 1:
        ...
        Gemma_file_name = "./exper/gemma_out/Gemma_QA_output_" + str(i) + ".txt"
        Gemma_file = open(Gemma_file_name, "w")
        Gemma_file.write(Gemma_out)
        Gemma_file.close()
        Gemma_time_name = "./exper/gemma_out/Gemma_QA_time_" + str(i) + ".txt"
    ...
    reference = doc['answer'][i]
    
    output_file = "./exper/InferenceResultFFT/QA_" + dataset_size + "_" + str(i) + ".out"
...
output_report = "./exper/InferenceReportFFT/QA_Inference_" + dataset_size + ".out"
