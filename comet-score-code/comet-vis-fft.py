...
def visualize_comet_scores_comparison(csv_file_adapter, csv_file_gemma, output_dir):
    ...
    merged_csv_path = f'{output_dir}/comet_scores_fft_20241015.csv'
    ...
    for translation_type in grouped_adapter['Translation_Type'].unique():
        ...
        for i, value in enumerate(subset_adapter['Comet_Score_mt_adapter']):
            plt.text(subset_adapter['Dataset_Size'].iloc[i], value, f'{value:.4f}', ha='center', va='bottom', fontsize=10)
        ...
        for i, value in enumerate(subset_gemma['Comet_Score_mt_gemma']):
            plt.text(subset_gemma['Dataset_Size'].iloc[i], value, f'{value:.4f}', ha='center', va='bottom', fontsize=10)
        ...
        # 그래프 저장 및 보여주기
        output_graph_path = f'{output_dir}/comet_score_comparison_{translation_type}_fft.png'
        plt.tight_layout()
