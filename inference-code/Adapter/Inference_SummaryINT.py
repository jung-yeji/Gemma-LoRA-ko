...
FINETUNE_MODEL = "./exper/MergeModel/summaryINT-gemma-2b-it-sum-ko.Rank" + str(rank) + "." + dataset_size
...
test_dataset = "dataset-SummaryINT-400"
...
for i in range(test_num):
    ...
    messages = [
        {
            "role": "user",
            "content": "다음 대화에서 답변을 요약해주세요:\n\n질문: {}\n\n답변: {}".format(doc['question'][i], doc['answer'][i])
        }
    ]
    ...
    if int(is_gemma_train) == 1:
        ...
        Gemma_file_name = "./exper/gemma_out/Gemma_SummaryINT_output_" + str(i) + ".txt"
        Gemma_file = open(Gemma_file_name, "w")
        Gemma_file.write(Gemma_out)
        Gemma_file.close()
        Gemma_time_name = "./exper/gemma_out/Gemma_SummaryINT_time_" + str(i) + ".txt"
    ...
    output_file = "./exper/InferenceResult/SummaryINT.Rank" + str(rank) + "." + dataset_size + "_" + str(i) + ".out"
...
output_report = "./exper/InferenceReport/SummaryINT_Inference.Rank" + str(rank) + "." + dataset_size + ".out"
