...
if __name__ == "__main__":
    ...
    json_train_dir = 'DATASET/022.요약문 및 레포트 생성 데이터/01.데이터/1.Training/라벨링데이터/02.briefing/20per'
    json_valid_dir = 'DATASET/022.요약문 및 레포트 생성 데이터/01.데이터/2.Validation/라벨링데이터/02.briefing/20per'
    ...
    if (max_files == None):
        filename = 'datasetREPORT-ALL'
    else:
        filename = 'datasetREPORT-'+str(max_files)
    dataset_dict.save_to_disk(filename)

    # Print a sample from the dataset
    print(dataset_dict)
    print(dataset_dict['train'][265])
